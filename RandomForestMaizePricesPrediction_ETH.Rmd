...
```
```{r }
maize.price <- read.csv("./ETH_maizeprices_LSMS.csv")
...
trf <- tuneRF(x=predict.vrbs, # Prediction variables
              y=maizeprice_laea@data[, "price_MT"] # Responce variable
              )
```
...
plot(spatial.prediction, main = "Predicted Maize Prices (ETB/KG)")
...
lm_line <- lm(non.spatial.prediction ~ maizeprice_laea@data[, "price_MT"], 
              data = data.frame(maizeprice_laea@data[, "price_MT"], 
                                non.spatial.prediction
                                )
              )
abline(lm_line)
